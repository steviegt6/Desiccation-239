    ...
    internal class MinerLightUI : UIState
    {
        ...
        public override void OnInitialize()
        {
            _vanillaItemSlot = new VanillaItemSlotWrapper(ItemSlot.Context.BankItem, 0.85f)
            {
                Left = { Pixels = 30 },
                Top = { Pixels = 270 },
                ValidItemFunc = item => item.IsAir || !item.IsAir && item.Prefix(-3) && item.pick > 0
            };
        ...
        // OnDeactivate is called when the UserInterface switches to a different state. In this mod, we switch between no state (null) and this state (MinerUI).
        ...
        public override void OnDeactivate()
        {
            ...
            // Note that in Miner we call .SetState(new UI.MinerUI());, thereby creating a new instance of this UIState each time. 
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (Main.LocalPlayer.talkNPC == -1 || Main.npc[Main.LocalPlayer.talkNPC].type != Desiccation.Instance.NPCType("Miner"))
            {
                ...
                Desiccation.Instance.MinerUserInterface.SetState(null);
            }
        }
